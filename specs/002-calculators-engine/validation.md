...
| SC-004 | Flag visible ≤ 1 s after rule eval | PASS *(reworded)* | FR-011 / SC-004 reworded to target the manual `POST /progression-flags/evaluate` round-trip in Phase 1 (Phase 4 wires session-save auto-trigger). Live evaluate returned in 561 ms. `GET /progression-flags` immediately afterwards returned the active flag set. |
