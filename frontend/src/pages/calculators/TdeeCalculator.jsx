...
const ACTIVITY_OPTIONS = [
  { value: 'sedentary', label: 'Sédentaire (1.2)' },
  { value: 'lightly_active', label: 'Légèrement actif (1.375)' },
  { value: 'moderately_active', label: 'Modérément actif (1.55)' },
  { value: 'very_active', label: 'Très actif (1.725)' },
  { value: 'extremely_active', label: 'Extrêmement actif (1.9)' },
];
...
export default function TdeeCalculator() {
  ...
  async function onSubmit(e) {
    ...
    setError(null);
    try {
      const res = await apiPost('/api/v1/calculators/tdee', form);
      setResult(res.data);
    } catch (err) {
      ...
      {result ? (
        <ResultCard title="Résultat">
          <dl className="grid grid-cols-2 gap-sm">
            ...
            <dd>{result.bmr_kcal} kcal/j</dd>
            <dt className="text-muted">TDEE</dt>
            <dd className="text-2xl font-semibold">{result.tdee_kcal} kcal/j</dd>
          </dl>
        </ResultCard>
      ) : null}
    </main>
